...
export class UnifiedDeduplicator {
  ...
  shouldSendAlert(
    ...
    // Less aggressive blocking: Only block exact same game state within 30 seconds
    if (isSameGameState && timeSinceFirst < 30000) {
      console.log(`🚫 DEDUP: Blocking duplicate ${alertKey.type} - exact same game state (30s window)`);
      console.log(`   ⏱️  Time since last: ${secondsSince}s`);
      return false;
    }

    // Reduced blocking: Only block same type alerts within 15 seconds for rapid-fire situations
    const isSameTypeAndGame = (
      cached.lastContext.gameId === alertKey.gameId &&
      cached.lastContext.type === alertKey.type
    );
    
    if (isSameTypeAndGame && timeSinceFirst < 15000) {
      console.log(`🚫 DEDUP: Blocking ${alertKey.type} - same type/game within 15s`);
      console.log(`   ⏱️  Time since last: ${secondsSince}s`);
      return false;
    }
