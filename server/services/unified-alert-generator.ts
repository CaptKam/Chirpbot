    ...
    try {
      ...
      const alertResults: SharedAlertResult[] = alerts.map(alert => ({
        ...
        context: {
          ...
          hasFirst: (gameState.runners as { first?: boolean })?.first,
          hasSecond: (gameState.runners as { second?: boolean })?.second,
          hasThird: (gameState.runners as { third?: boolean })?.third,
          outs: gameState.outs,
          balls: gameState.balls,
          strikes: gameState.strikes,
          currentBatter: (gameState.currentBatter as { name?: string })?.name,
          onDeckBatter: (gameState.onDeckBatter as { name?: string })?.name,
          currentPitcher: (gameState.currentPitcher as { name?: string })?.name,
