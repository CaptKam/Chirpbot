/**
 * CFL Event Stream Processor
 * 
 * Preserves ALL existing CFL alert logic while providing event-driven
 * processing capabilities. Integrates with existing CFLEngine, alert
...
// Import existing CFL system components
import { CFLEngine } from '../engines/cfl-engine';
import { getUnifiedSettings } from '../unified-settings';

// Import specific CFL alert modules for direct access
import FinalMinutesModule from '../engines/alert-cylinders/cfl/final-minutes-module';
import FourthQuarterModule from '../engines/alert-cylinders/cfl/fourth-quarter-module';
import GameStartModule from '../engines/alert-cylinders/cfl/game-start-module';
import GreyCupImplicationsModule from '../engines/alert-cylinders/cfl/grey-cup-implications-module';
import MassiveWeatherModule from '../engines/alert-cylinders/cfl/massive-weather-module';
import OvertimeModule from '../engines/alert-cylinders/cfl/overtime-module';
import RougeOpportunityModule from '../engines/alert-cylinders/cfl/rouge-opportunity-module';
import SecondHalfKickoffModule from '../engines/alert-cylinders/cfl/second-half-kickoff-module';
import ThirdDownSituationModule from '../engines/alert-cylinders/cfl/third-down-situation-module';
import TwoMinuteWarningModule from '../engines/alert-cylinders/cfl/two-minute-warning-module';

export class CFLProcessor extends BaseProcessor {
  private readonly cflEngine: CFLEngine;
  ...
  constructor(id: string = 'cfl_processor', sport: string = 'CFL') {
    super(id, sport);
    
    // Initialize with existing CFL engine
    this.cflEngine = new CFLEngine();
    
    // Initialize all CFL alert modules
    this.initializeAlertModules();
    
    console.log(`🏈 CFL Event Stream Processor initialized with ${this.alertModules.size} alert modules`);
  }

  /**
   * Initialize all CFL alert modules (same as existing system)
   */
  private initializeAlertModules(): void {
    const modules = [
      { name: 'CFL_FINAL_MINUTES', module: FinalMinutesModule },
      { name: 'CFL_FOURTH_QUARTER', module: FourthQuarterModule },
      { name: 'CFL_GAME_START', module: GameStartModule },
      { name: 'CFL_GREY_CUP_IMPLICATIONS', module: GreyCupImplicationsModule },
      { name: 'CFL_MASSIVE_WEATHER', module: MassiveWeatherModule },
      { name: 'CFL_OVERTIME', module: OvertimeModule },
      { name: 'CFL_ROUGE_OPPORTUNITY', module: RougeOpportunityModule },
      { name: 'CFL_SECOND_HALF_KICKOFF', module: SecondHalfKickoffModule },
      { name: 'CFL_THIRD_DOWN_SITUATION', module: ThirdDownSituationModule },
      { name: 'CFL_TWO_MINUTE_WARNING', module: TwoMinuteWarningModule }
    ];
  ...
  async processGameStateChanged(event: GameStateChangedEvent, context: ProcessorContext): Promise<ProcessorResult> {
    ...
    try {
      ...
      // Delegate to existing CFL engine for alert generation
      const alerts = await this.cflEngine.generateLiveAlerts(event.payload.currentState);
      ...
      // Shadow mode logging
      if (this.config.shadowMode) {
        this.log('info', `[Shadow][cfl_processor] Processed ${filteredAlerts.length} alerts for game ${event.payload.gameId} in ${processingTime}ms`);
        
        if (filteredAlerts.length > 0) {
          this.log('info', `[Shadow][cfl_processor] Alert types: ${filteredAlerts.map(a => a.type).join(', ')}`);
        }
      }
      ...
      const processingTime = Date.now() - startTime;
      this.log('error', `Error processing CFL game state change:`, error);
  ...
  async processAlertGenerated(event: AlertGeneratedEvent, context: ProcessorContext): Promise<ProcessorResult> {
    ...
    if (this.config.shadowMode) {
      this.log('debug', `[Shadow][cfl_processor] Received alert: ${event.payload.alertResult.type} for game ${event.payload.gameId}`);
    }
  ...
  /**
   * Core alert generation logic - preserves existing CFL engine behavior
   */
  protected async generateAlerts(gameState: GameState, context: ProcessorContext): Promise<AlertResult[]> {
    try {
      // Delegate to existing CFL engine for alert generation
      const alerts = await this.cflEngine.generateLiveAlerts(gameState);
      ...
      if (this.config.shadowMode && filteredAlerts.length > 0) {
        this.log('info', `Generated ${filteredAlerts.length} CFL alerts for game ${gameState.gameId}`);
      }

      return filteredAlerts;
      
    } catch (error) {
      this.log('error', `Error generating CFL alerts:`, error);
      return [];
    }
  }
