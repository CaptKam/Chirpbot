...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Admin middleware - check if user is authenticated and has admin role
  ...
  app.get('/api/admin/users', requireAdmin, async (req, res) => {
    try {
      ...
      const sanitizedUsers = allUsers.map((user: any) => {
        const { password, ...userWithoutPassword } = user;
  ...
  app.get('/api/admin/stats', requireAdmin, async (req, res) => {
    try {
      const [usersResult, alertsResult, todayAlertsResult] = await Promise.all([
        db.execute(sql`SELECT COUNT(*) as count FROM users`),
        db.execute(sql`SELECT COUNT(*) as count FROM alerts`),
        db.execute(sql`SELECT COUNT(*) as count FROM alerts WHERE DATE(created_at) = CURRENT_DATE`)
      ]);
      ...
      const stats = {
        ...
        todayAlerts: parseInt(todayAlertsResult.rows[0]?.count || '0'),
        liveGames: 0, // This would need to be calculated from current live games
        activeUsers: 1 // This would need WebSocket connection tracking
      };
  ...
  app.post('/api/admin/users/:userId/role', requireAdmin, async (req, res) => {
    try {
      ...
      if (!['user', 'analyst', 'manager', 'admin'].includes(role)) {
