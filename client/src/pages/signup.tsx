...
import { Zap, User, Lock, Mail, ArrowLeft, CheckCircle, AlertCircle } from "lucide-react";
...
export default function Signup() {
  ...
  // Inline validation (Nielsen Heuristic 5: Error Prevention)
  ...
  const validateField = useCallback((name: string, value: string) => {
    ...
    if (name === 'usernameOrEmail') {
      if (!value.trim()) setError(name, 'Please enter a username or email');
      else if (value.length < 3) setError(name, 'Must be at least 3 characters');
      else clearError(name);
    }
    if (name === 'password') {
      if (!value) setError(name, 'Please enter a password');
      else if (value.length < 6) setError(name, 'Password must be at least 6 characters');
      else clearError(name);
    }
    if (name === 'confirmPassword') {
      if (!value) setError(name, 'Please confirm your password');
      else if (value !== password) setError(name, 'Passwords do not match');
      else clearError(name);
    }
  }, [password]);
  ...
  const signupMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description: "Your ChirpBot account has been created successfully.",
      });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!usernameOrEmail.trim() || usernameOrEmail.length < 3 || !password || password.length < 6 || password !== confirmPassword) {
      return;
    }
  ...
  // Helper to render field with inline validation
    ...
    <div className="min-h-screen bg-solidBackground flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-white mb-2" style={{ lineHeight: 1.2 }}>Join ChirpBot</h1>
          <p className="text-slate-300" style={{ fontSize: '17px', lineHeight: 1.5 }}>Get real-time sports alerts powered by AI</p>
        </div>
        ...
        <Card className="glass-card border-0 shadow-xl">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              ...
              {/* Username/Email — inline validation */}
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className={inputClass('usernameOrEmail')}
                    disabled={isLoading}
                  ...
                  />
                  {renderFieldStatus('usernameOrEmail')}
                </div>
              ...
              <div className="space-y-2">
                ...
                {fieldErrors.confirmPassword && (
                  <p id="confirmPassword-error" className="field-error-message flex items-center gap-1.5" role="alert">
                    <AlertCircle className="h-3.5 w-3.5 flex-shrink-0" aria-hidden="true" />
                    {fieldErrors.confirmPassword}
                  </p>
                )}
            ...
            <div className="space-y-3">
              <Button
                variant="outline"
                className="w-full min-h-[48px] bg-white/[0.04] border-white/[0.12] text-slate-200 hover:bg-white/[0.08] hover:border-white/[0.2]"
